...
### 3.人工智能概率性考量
- 百度AI人流量识别功能使用高精度头肩检测算法，准确率90%以上，统计人数无上限，适应各种人群密集场所，且服务稳定，拥有精确的大流量服务，可靠性高，保障高达99%；环境中的遮挡物会干扰识别结果，且对摄像头架设的角度有一定要求，但这些都可以通过设定区域来排除干扰，提升识别效果，不会对用户产生较大的负面影响。
...
### 9.需求列表与人工智能API加值
| 优先级 | 用户需求 | 智能加值| API类型 |如何加值|重要程度|
|------| ------| ------ | ------ |------ |------|
| 1 | 不知道哪里厕所的人流量相对较小 | 是| 百度AI开放平台、阿里云开放平台---人流量统计API |根据返回结果显示地点的人流量，告知用户人流量最少的目的地|重要|
| 2| 不清楚周围哪里有厕所| 是 | 高德开放平台、百度地图开放平台---输入提示API |返回结果显示用户当前位置周围搜索的地点，通过地图显示|重要|
| 3 | 不知道厕所的具体位置  | 是| 高德开放平台、百度地图开放平台---路线规划API |返回结果显示用户当前位置到目的地的路线规划|次重要|
| 4 | 当前在什么位置 | 否 |手机地理定位|显示用户当前位置，通过地图显示|次重要|

### 10.利害相关者分析
- 可持续性发展： 在“找厕所”APP中，使用到高德开放平台和百度AI开放平台的API，这两大平台的可靠性保障高，精确度高，知名度高，具有一定权威性，对该APP的使用也有很大的支持作用，在一定的程度上促进了“找厕所”APP的可持续性发展。调用输入提示API、人流量统计API、路径规划API，主要是利用人流量统计和距离综合判断为用户推荐更便捷的目的地，节省用户时间，缓解严重的排队现象，提高用户旅行体验。
- ESG考量： “找厕所”APP的主要用户为女性，因为考虑到男性用户旅游在外对找厕所的需求相对较小，但非常欢迎男性用户使用，在后期也会增加旅行攻略推荐和周边产品预定服务，使该APP面对的用户更大众化。
...
#### 两种API的功能都符合于“找厕所”APP的主要需求，也许调用阿里云开放平台的人数检测OCR会更容易些，但考虑到产品的商用，还有百度AI的人流量统计API对于人流量的统计无人数上限，且准确率90%以上，对于“找厕所”这款主要使用在旅游景区的产品是更适用的，且对比价格，还有百度AI开放平台的文档完善度高，保障性高；所以在短期内会使用百度AI开放平台的人流量统计API。
...
|技术可行性| 高德开放平台和百度AI开放平台这两大平台可靠性保障高，精确度高，具有一定的权威性；且能及时对用户的反馈进行分析总结，强化机器学习。| 
...
## 4.心得总结与感谢
- 万事开头难，整个项目中第一步是最艰难的，要构思好整个流程。我认为最好的方法是从用户痛点出发，首先锁定某个范围的用户，然后分析用户需求，想出方案解决用户需求。在本项目中调用了高德开放平台的输入提示API、路径规划API，和百度AI开放平台的人流量分析API，这两大平台的易用程度高，保障性高，精确度高，对整个项目的完成度有很大的帮助。
